...
For text, `orientation` is `genes_by_cells` or `cells_by_genes`; `delimiter` is `comma`, `tab`, or `space`; `feature_column` is the first-axis identifier column (genes or cells depending on orientation). When an inspected table has exactly one fewer header field than data fields, use the explicit sentinel `feature_column=__row_names__`; the reader accepts either R's synthetic `row.names` column or actual row names for this reviewed layout. Never use the sentinel for an ordinary labeled identifier column. `drop_columns` is an optional semicolon-separated list of explicitly inspected nonexpression columns. The loader rejects remaining nonnumeric data, duplicate IDs and missing counts. Ambiguous orientation requires evidence and a report update; do not infer from matrix dimensions alone. Dense text import has substantial RAM overhead.
