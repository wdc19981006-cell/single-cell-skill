...
`download_profile.csv` records bytes, transfer seconds, MB/s (decimal), actual retries, REUSED/DOWNLOADED/FAILED, SHA seconds and archive extraction seconds for each source. `decision_trace.md` includes observed text-schema and pooled-mapping probe results. `input_routing.csv` records format, reader, unique physical inputs, expression and cell-map reads, dimensions, read seconds and dense conversion. `build_profile.json` records read, mapping, combine, CreateSeuratObject, validation, saveRDS, total time and obtainable memory estimates. A missing stage in a failed run is explicitly marked unavailable.
