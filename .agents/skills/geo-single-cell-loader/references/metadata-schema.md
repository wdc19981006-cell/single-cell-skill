...
`.workflow/download.json` temporarily retains records per destination, including `url`, `sha256`, `bytes`, `downloaded_at`, `local_path`. The audit's `download_profile.csv` preserves this provenance and is restored into a fresh temporary file before future reuse. Archive member records additionally bind `member`, `extracted_sha256`, `extracted_bytes` to the source archive checksum. Archives are cached under `raw/_archives/`. Existing inputs with declared download plans require matching provenance; `files_json=[]` is only for reviewed local inputs, never a workaround for a missing/mismatched download record.
